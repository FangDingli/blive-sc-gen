...
watchEffect(() => {
  let imageName = `face-frame-${props.userRole}`
  if (props.overTenKGuard) {
    imageName += '-w'
  } else if (props.isThousandsGuard) {
    imageName += '-k'
  }
  faceFrame = new URL(`../assets/images/${imageName}.png`, import.meta.url).href
})
</script>
